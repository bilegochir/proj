...
import (
	...
	"github.com/bilegochir/proj/support"

	// need to pull in the operations table entries
	_ "github.com/bilegochir/proj/operations"
)
...
func newConversion(dest EPSGCode) (*conversion, error) {
	...
	if !ok {
		return nil, fmt.Errorf("epsg code is not a supported projection: %v", dest)
	}
...
func (conv *conversion) convert(input []float64) ([]float64, error) {
	...
	lp := &core.CoordLP{}
	lp.Lam = support.DDToR(input[0])
	lp.Phi = support.DDToR(input[1])
